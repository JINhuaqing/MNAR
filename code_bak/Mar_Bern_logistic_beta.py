...
from utilities_mar import MarBetaBern
import random
import numpy as np
import torch
import pickle
...
Cb_adj = 0.01 #0.01/2  # constant before beta algorithm lambda_beta , should be smaller than 1 
...
betahat, numI, Berrs, Likelis, betahats = MarBetaBern(1000, X, Y, R, conDenfs, TrueParas=TrueParas, Cb=Cb, tol=tol, log=2, prob=prob, betainit=betainit, ErrOpts=1, etabs=etabs, etabsc=etabsc)
...
# Save the output
f = open(f"./outputs/Mar_Bern_beta_{m}_{Cb_adj:.0E}_{etabs[-1]:.0E}.pkl", "wb")
