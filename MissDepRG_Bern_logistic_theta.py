...
for i in range(numRG):
    ...
    try:
        bThetahat, numI, Terrs = BthetaBern(1000, X, Y, R, conDenfs, TrueParas=TrueParas, CT=CT, tol=tol, log=0, prob=prob, bThetainit=bThetainit, ErrOpts=1)
    except RuntimeError as e:
        results.append((-100,  CT, -100, -100, -100))
        Errs.append([])
        print(
            f"The {i+1}th/{numRG},"
            f"Iteration Fails!", 
        ...
        results.append((numI, CT, errT.item(), bThetahat.norm().item(), LpTTvhat.min().item()))
        Errs.append(Terrs)
        print(
            f"The {i+1}th/{numRG},"
...
# Save the output
f = open("./outputs/RandGrid_Bern_theta_100.pkl", "wb")
