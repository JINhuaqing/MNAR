...
def EML(bTheta, beta, f, X, Y, is_logf=False):
    """
    bTheta: the matrix parameter, n x m
    beta: the vector parameter, p 
...
# function to optimize on realdata of MAR method  when Y is binary
def EMRealDataAlg(MaxIters, X, Y, R, conDenfs, Cb=10, CT=1, etab=1e-3, etaT=1, log=0, bThetainit=None, betainit=None, tols=None, prob=0.5, ErrOpts=0):
    """
    ...
    for t in range(MaxIters):
        ...
        LvNow = EML(bThetaOld, betaOld, f, X, Y, is_logf=is_logf)
        ...
        betaNewRaw = betaOld-etab * EMLpb(bThetaOld, betaOld, conDenfs[:3], X, Y)
        ...
        #--------------------------------------------------------------------------------
        # Update bTheta 
        LpTvOld = EMLpT(bThetaOld, betaNew, conDenfs[:3], X, Y)
        ...
        if t % 10 == 0:
            if log==1:
                ...
                print(tb2)
            if log==2:
                tb2 = PrettyTable(["Iter", "etaT", "etab", "Loss", "-lkd", "reCh", "BNorm ", "TNorm ", "Bdiff", "Tdiff", "bL0norm"])
