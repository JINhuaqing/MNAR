...
for i in range(numSimu):
    ...
    try:
        betahat, bThetahat, numI, Berrs, Terrs, betahats, bThetahats, Likelis, etass = NewBern(50000, X, Y, R, sXs, conDenfs, TrueParas=TrueParas, Cb=Cb, CT=CT, tols=tols, log=loglv, prob=prob, betainit=betainit, bThetainit=bThetainit, ErrOpts=1, etab=etab, etaT=etaT)
...
# Save the output
f = open(f"./outputs/SimulationLogistic_p{p}_{m}.pkl", "wb")
