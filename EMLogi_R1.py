from utils import *
from utils_EM import EMNewBernLogi
import random
import numpy as np
import torch
import pickle
...
from confs import fln, fln2_raw, fln22
...
if m == 100 and p == 50: 
    ...
    Cb, CT = 800, 8e-2
    etab, etaT = 0.07, 0.105
elif m == 200 and p == 50:
    Cb, CT = 1000, 8e-2
    etab, etaT = 0.10, 0.11
elif m == 200 and p == 100:
    Cb, CT = 1000, 8e-2
    etab, etaT = 0.13, 0.095
elif m == 400 and p == 50:
    Cb, CT = 800, 8e-2
    etab, etaT = 0.40, 0.11
    ...
    Cb, CT = 800, 8e-2
    etab, etaT = 1.90, 0.103
elif m == 1600 and p == 50:
    Cb, CT = 800, 8e-2; 
    etab, etaT = 6.00, 0.095
elif m == 1600 and p == 100:
    Cb, CT = 800, 8e-2; 
    etab, etaT = 7.00, 0.105
...
for i in range(numSimu):
    ...
    if (i+1) >= startIdx:
        betahat, bThetahat, numI, Berrs, Terrs, betahats, bThetahats, Likelis = EMNewBernLogi(1000, X, Y, R, conDenfs, TrueParas=TrueParas, etab=etab, Cb=Cb, CT=CT, tols=tols, log=loglv, prob=prob, betainit=betainit, bThetainit=bThetainit, ErrOpts=1, etaT=etaT)
        ...
        filName = f"EM_logi_p{p}_m{m}_simu{i+1}_iter{numI}.pkl"
