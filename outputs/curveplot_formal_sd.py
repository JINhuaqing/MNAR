...
for f in files:
    ...
    Berr, Terr = np.std(resarr, axis=0)[[2, 5]]
    Berrs.append(Berr)
    Terrs.append(Terr)
...
plt.legend(loc=1)
if typ.startswith("log"):
    plt.savefig(f"{p}_MNARxMAR_sd_beta_Logistic.jpg")
else:
    plt.savefig(f"{p}_MNARxMAR_sd_beta_Linear.jpg")
plt.close()
...
plt.legend(loc=1)
if typ.startswith("log"):
    plt.savefig(f"{p}_MNARxMAR_sd_Theta_Logistic.jpg")
else:
    plt.savefig(f"{p}_MNARxMAR_sd_Theta_Linear.jpg")
plt.close()
